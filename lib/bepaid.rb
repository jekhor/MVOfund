...
#  MANDATORY_ATTRS = [:amount, :currency, :description, :order_id, :email, :ip, :payment_method]
  ...
  class BePaid
    def initialize(base_url, user, password)
      @base_url = base_url
      @base_checkout_url = base_url.gsub('api', 'checkout')
    ...
    def post_checkout(checkout)
      r = query_checkout_api :post, '/ctp/api/checkouts', checkout
      checkout_status = JSON.parse r.body
      raise BPError, checkout_status['message'] if checkout_status.include? 'errors'
      checkout_status
    end

    def get_checkout_status(token)
      r = query_checkout_api :get, "/ctp/api/checkouts/#{token}"
      checkout_status = JSON.parse r.body
      checkout_status
    end
    ...
    def query_checkout_api(method, path, body = nil)
    ...
    def query_api(method, path, body = nil)
      query_api_common(@base_url, method, path, body)
    end

    def query_api_common(base_url, method, path, body = nil)
      body = body.to_json unless body.nil?
      raise RuntimeError.new "No credentials for bePaid" if @auth[:username].nil? or @auth[:username] == ''
      begin
        r = RestClient::Request.execute method: method,
